<div>
    <x-slot name="header">Hasil Penempatan</x-slot>
    ...
    <div class="mb-6">
        <nav class="flex items-center gap-2 text-sm text-gray-500">
            ...
            <a href="{{ route('admin.kegiatan-ujian.acak-kelas', $kegiatanUjian->id) }}" class="hover:text-gray-700"
                wire:navigate>Acak Kelas</a>
            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            </svg>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">
                    Pasangan: <span class="font-medium">{{ $pasangan->kelas_a_nama }}</span> ↔
                    <span class="font-medium">{{ $pasangan->kelas_b_nama }}</span>
                </p>
            </div>
            <div class="flex items-center gap-3">
                <span class="badge badge-primary">{{ $totalSiswa }} siswa ditempatkan</span>
                <a href="{{ route('admin.kegiatan-ujian.acak-kelas', $kegiatanUjian->id) }}" class="btn btn-secondary"
                    wire:navigate>
                    ...
                    <tbody>
                        @foreach ($penempatans as $penempatan)
                            <tr>
                                ...
                                <td>
                                    @if ($penempatan->asal_kelas === $pasangan->kelas_a_nama)
                                        <span class="badge badge-primary">{{ $penempatan->asal_kelas }}</span>
                                    @else
                                        <span class="badge badge-secondary">{{ $penempatan->asal_kelas }}</span>
                                    @endif
                                </td>
